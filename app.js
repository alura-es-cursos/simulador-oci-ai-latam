...
`;
const HEADER_TITLE_HTML = 'Simulator Exam - Oracle Cloud Infrastructure (1Z0-1122-26) <strong>OCI AI Foundations Associate</strong>';
...
function renderSharedHeaders() {
  document.querySelectorAll('.header-logo').forEach((el) => { el.innerHTML = HEADER_LOGO_SVG; });
  document.querySelectorAll('.header-title').forEach((el) => { el.innerHTML = HEADER_TITLE_HTML; });
}
...
function startExam() {
  ...
  state.answers = state.examQuestions.map(() => ({ selected: null, marked: false }));
...
function renderQuestion() {
  ...
  if (state.mode === 'review') {
    const isCorrect = answer.selected !== null && item.options[answer.selected].isCorrect;
  ...
  btnNext.textContent = atEnd ? (state.mode === 'review' ? 'Back to Results' : 'Finish') : 'Next';
}
...
function showSummary() {
  ...
  if (unansweredIdx.length > 0) {
    ...
    msg.textContent = 'Has respondido todas las preguntas. Puedes enviar el examen cuando quieras.';
...
function finishExam() {
  ...
  if (ratio < PASS_THRESHOLD) {
    badge.textContent = 'No aprobaste. Continúa practicando, revisa las respuestas incorrectas y continúa estudiando.';
    badge.classList.add('not-approved');
  } else if (ratio < IMPROVE_THRESHOLD) {
    badge.textContent = '¡Felicidades! Aprobaste el simulado, pero puedes mejorar tu puntaje. Revisa las respuestas incorrectas — vale la pena el esfuerzo.';
    badge.classList.add('improve');
  } else if (ratio < OFFICIAL_EXAM_THRESHOLD) {
    badge.textContent = '¡Felicidades! Aprobaste el simulado. Con un poco más de práctica puedes alcanzar el 80%.';
    badge.classList.add('ready');
  } else {
    badge.textContent = '¡Felicidades! Estás listo para la prueba real en Oracle myLearn.';
    badge.classList.add('ready');
  }
...
function showIncorrectScreen() {
  ...
  const label = incorrectIdx.length === 1 ? 'pregunta incorrecta' : 'preguntas incorrectas';
  ...
  incorrectIdx.forEach((idx) => {
    grid.appendChild(createGridCell(idx + 1, 'Incorrect', 'sg-incorrect', () => renderIncorrectDetail(idx)));
  });
...
async function initApp() {
  ...
  byId('start-q-count').textContent = Math.min(EXAM_LENGTH, state.questionBank.length);
  byId('start-duration').textContent = formatDurationLabel(EXAM_DURATION_SECONDS);
...
function bindEvents() {
  ...
  byId('btn-back').addEventListener('click', () => {
    const firstUnansweredIdx = state.answers.findIndex((a) => a.selected === null);
  ...
  byId('btn-incorrect-close').addEventListener('click', () => showScreen('results-screen'));
}
